function instantaneous_values = wave_prd_inst_f(t,eta,window_size)
...
% for ii = window_size+1:10+window_size
%         eta_est = eta_prd(t(ii-window_size:ii),eta(ii-window_size:ii),window_size,'regression');
%         temp_hilbert = hilbert(eta_est);
%         temp_mag = abs(temp_hilbert);
%         temp_phase = angle(temp_hilbert);
%         mag(ii) = temp_mag(window_size+1);
%         phase(ii) = temp_phase(window_size+1);
% end




for ii = window_size+1:length(t)-window_size
        eta_est = eta_prd(t(ii-window_size:ii),eta(ii-window_size:ii),window_size,'regression');
        temp_hilbert = hilbert(eta_est);
        temp_mag = abs(temp_hilbert);
        temp_phase = angle(temp_hilbert);
        mag(ii) = temp_mag(window_size);
        phase(ii) = temp_phase(window_size);
end

for ii = length(t)-window_size+1:length(t)    
        eta_est = eta_prd(t(ii-window_size:ii),eta(ii-window_size:ii),window_size,'regression');
        temp_hilbert = hilbert(eta_est);
        temp_mag = abs(temp_hilbert);
        temp_phase = angle(temp_hilbert);
...
mag_filtered = movmedian(mag,window_size);
inst_freq_filtered = movmedian(inst_freq,window_size);

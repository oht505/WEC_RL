function instantaneous_values = semi_cheat_inst_f(t,eta,window_size)
...
for ii = window_size+1:length(t)-window_size
        temp_hilbert = hilbert(eta(ii-window_size:ii+window_size));
...
for ii = length(t)-window_size+1:length(t)    
        temp_hilbert = hilbert(eta(ii-window_size:ii));
        temp_mag = abs(temp_hilbert);
        temp_phase = angle(temp_hilbert);
        mag(ii) = temp_mag(end);
...
mag_filtered = movmedian(mag,window_size*dt);
inst_freq_filtered = movmedian(inst_freq,window_size*dt);
